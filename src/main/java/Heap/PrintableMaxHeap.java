...
public class PrintableMaxHeap extends MaxHeap{
    ...
    public void treePrint(){
        ...
        System.out.println("The max heap size is: " + getCount());
        System.out.println("Data in the max heap: ");
        for( int i = 1 ; i <= getCount() ; i ++ ){
            // 我们的print函数要求堆中的所有整数在[0, 100)的范围内
            assert (Integer)mContent[i] >= 0 && (Integer)mContent[i] < 100;
            System.out.print(mContent[i] + " ");
        }
        System.out.println();
        System.out.println();
        ...
        for( int level = 0 ; level < maxLevel ; level ++ ){
            ...
            int curLevelNumber = Math.min(mCount-(int)Math.pow(2,level)+1,(int)Math.pow(2,level));
            boolean isLeft = true;
            for( int indexCurLevel = 0 ; indexCurLevel < curLevelNumber ; index ++ , indexCurLevel ++ ){
                line1 = putNumberInLine( (Integer)mContent[index] , line1 , indexCurLevel , curTreeMaxLevelNumber*3-1 , isLeft );
                isLeft = !isLeft;
            }
